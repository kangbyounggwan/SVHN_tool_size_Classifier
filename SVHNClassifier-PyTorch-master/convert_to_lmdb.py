...
parser.add_argument('-d', '--data_dir', default='C:\\Users\\ST200423\\Desktop\\svhn', help='directory to SVHN (format 1) folders and write the converted files')
...
def main(args):
    path_to_train_dir = os.path.join(args.data_dir, 'dataset')
    path_to_test_dir = os.path.join(args.data_dir, 'test')
    path_to_val_dir = os.path.join(args.data_dir, 'val')
    ...
    path_to_extra_digit_struct_mat_file = os.path.join(path_to_val_dir, 'digitStruct.mat')
    ...
    [num_train_examples, num_val_examples] = convert_to_lmdb([(path_to_train_dir, path_to_train_digit_struct_mat_file),
                                                              (path_to_val_dir, path_to_extra_digit_struct_mat_file)],
