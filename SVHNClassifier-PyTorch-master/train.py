...
parser.add_argument('-d', '--data_dir', default='C:\\Users\\ST200423\\Desktop\\svhn', help='directory to read LMDB files')
parser.add_argument('-l', '--logdir', default='./logs', help='directory to write logs')
parser.add_argument('-r', '--restore_checkpoint', default='.\\logs\\model-54000.pth',
                    help='path to restore  , e.g. ./logs/model-100.pth')
...
def _train(path_to_train_lmdb_dir, path_to_val_lmdb_dir, path_to_log_dir,
    ...
    evaluator_train = Evaluator(path_to_train_lmdb_dir)
    ...
    while True:
        for batch_idx, (images, length_labels, digits_labels) in enumerate(train_loader):
            ...
            accuracy_val = evaluator.evaluate(model)
            accuracy_train = evaluator_train.evaluate(model)
            print('==> accuracy = %f, best accuracy %f' % (accuracy_val, best_accuracy))
            wandb.log({'accuracy_val': accuracy_val,'accuracy_train': accuracy_train ,'loss': loss.item() , 'learning_rate': scheduler.get_lr()[0] }, step= step)
...
def main(args):
    wandb.init(project='svhn_retrain')
    path_to_train_lmdb_dir = os.path.join(args.data_dir, 'data.lmdb')
